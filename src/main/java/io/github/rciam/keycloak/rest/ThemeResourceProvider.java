...
//    }
    ...
    public List<RciamIdentityProvider> getIdentityProviders(
        ...
        return session.identityProviders().getAllStream(searchOptions, firstResult, maxResults).map(idp -> createIdentityProvider(realm, UriBuilder.fromUri(session.getContext().getUri().getBaseUri().getPath()).build(), idp)).toList();
    }
